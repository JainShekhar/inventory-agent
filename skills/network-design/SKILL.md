...
For network design optimization, use `PuLP` for facility location and flow decisions, `stockpyl` for inventory positioning:

```python
import pulp

# Network design is typically a MIP combining:
# 1. Facility location (which nodes to open) — binary variables
# 2. Flow allocation (how much flows where) — continuous variables
# 3. Inventory positioning (how much stock at each node) — stockpyl

# Example: Minimize total network cost
prob = pulp.LpProblem("NetworkDesign", pulp.LpMinimize)
# ... see facility-location-problem and distribution-center-network skills
# for complete formulations

# After solving network structure, position inventory:
from stockpyl.gsm_tree import optimize_committed_service_times
from stockpyl.ssm_serial import optimize_base_stock_levels
from stockpyl.meio_general import meio_by_coordinate_descent
```

Use the skill's frameworks below for strategic decisions (number of echelons, flow strategies, make-vs-buy) that precede the mathematical optimization.
