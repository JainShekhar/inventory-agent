...
## Preferred Solver: PuLP (MIP)

For computation, use `PuLP` with the CBC solver (included). Facility location is a Mixed-Integer Program:

```python
import pulp
import numpy as np

def solve_capacitated_facility_location(
    fixed_costs,          # f[j]: cost to open facility j
    capacities,           # cap[j]: capacity of facility j
    demands,              # d[i]: demand of customer i
    transport_costs       # c[i][j]: cost to serve customer i from facility j
):
    ...
    # Decision variables
    y = [pulp.LpVariable(f"open_{j}", cat="Binary") for j in range(n_facilities)]
    x = [[pulp.LpVariable(f"flow_{i}_{j}", lowBound=0) for j in range(n_facilities)]
         for i in range(n_customers)]

    # Objective: minimize fixed + transport costs
    prob += (pulp.lpSum(fixed_costs[j] * y[j] for j in range(n_facilities)) +
             pulp.lpSum(transport_costs[i][j] * x[i][j]
                        for i in range(n_customers) for j in range(n_facilities)))

    # Constraints: meet all demand
    for i in range(n_customers):
        prob += pulp.lpSum(x[i][j] for j in range(n_facilities)) == demands[i]

    # Constraints: respect capacity (only if open)
    for j in range(n_facilities):
        prob += pulp.lpSum(x[i][j] for i in range(n_customers)) <= capacities[j] * y[j]

    prob.solve(pulp.PULP_CBC_CMD(msg=0))

    open_facilities = [j for j in range(n_facilities) if y[j].value() > 0.5]
    total_cost = pulp.value(prob.objective)
    flows = [[x[i][j].value() for j in range(n_facilities)] for i in range(n_customers)]

    return open_facilities, flows, total_cost
```

For inventory positioning at selected facilities, combine with stockpyl:
```python
from stockpyl.gsm_tree import optimize_committed_service_times  # safety stock placement
from stockpyl.meio_general import meio_by_coordinate_descent     # multi-echelon optimization
```
