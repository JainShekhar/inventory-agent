...
## Preferred Solver: PuLP (LP) + scikit-learn (clustering)

For allocation optimization, use `PuLP`. For store clustering, use `scikit-learn`:
...
def allocate_inventory_to_stores(
    total_supply,         # total units available
    store_forecasts,      # forecast demand per store
    store_capacities,     # max units each store can hold
    store_priorities,     # weight per store (e.g., revenue/sqft)
    min_allocation=0      # minimum units per store (MOQ)
):
    """Allocate inventory to stores maximizing weighted fill rate."""
    n_stores = len(store_forecasts)
    prob = pulp.LpProblem("StoreAllocation", pulp.LpMaximize)

    # Units allocated to each store
    x = [pulp.LpVariable(f"alloc_{i}", lowBound=min_allocation,
                          upBound=store_capacities[i], cat="Integer")
         for i in range(n_stores)]

    # Maximize priority-weighted allocation (capped at forecast)
    prob += pulp.lpSum(store_priorities[i] * x[i] for i in range(n_stores))

    # Total supply constraint
    prob += pulp.lpSum(x[i] for i in range(n_stores)) <= total_supply

    # Don't over-allocate vs forecast
    for i in range(n_stores):
        prob += x[i] <= store_forecasts[i]

    prob.solve(pulp.PULP_CBC_CMD(msg=0))
    return [int(x[i].value()) for i in range(n_stores)]

# Store clustering for allocation tiers
def cluster_stores(store_features, n_clusters=4):
    """Cluster stores by sales velocity, size, demographics."""
    km = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
    labels = km.fit_predict(store_features)
    return labels, km.cluster_centers_
```

For determining total supply (how much to buy for the season):
```python
from stockpyl.newsvendor import newsvendor_normal  # critical fractile for seasonal buys
```
