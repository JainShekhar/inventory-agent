...
For computation, use the `stockpyl` library. It provides a validated Wagner-Whitin solver:

```python
from stockpyl.wagner_whitin import wagner_whitin

# Solve dynamic lot-sizing with time-varying demand
# Parameters can be scalars (constant) or lists (time-varying)
Q, cost, theta, s = wagner_whitin(
    num_periods=12,
    holding_cost=2,          # or list: [2, 2, 3, 3, ...]
    fixed_cost=500,          # or list: [500, 500, 600, ...]
    demand=[90, 120, 80, 70, 50, 100, 60, 40, 80, 20, 110, 90],
    purchase_cost=0          # or list for time-varying prices
)
# Returns: order_quantities, optimal_cost, costs_to_go, next_order_periods
```

Use the skill's custom code below only for scenarios stockpyl doesn't cover (e.g., rolling horizon simulation, price speculation analysis, capacity-constrained lot sizing).
