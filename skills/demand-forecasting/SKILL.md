...
## Preferred Solver: scipy/numpy (available now) + statsmodels/Prophet (install as needed)

For forecasting, use what's available. Core methods with scipy/numpy only:
...
def exponential_smoothing(data, alpha=0.3):
    """Single exponential smoothing for level estimation."""
    forecast = [data[0]]
...
def decompose_and_forecast(data, season_length, periods_ahead):
    """Simple additive decomposition forecast."""
    n = len(data)
    # Trend via linear regression
    x = np.arange(n)
    slope, intercept, _, _, _ = stats.linregress(x, data)
    trend = intercept + slope * np.arange(n + periods_ahead)
    # Seasonal component (average deviation per season position)
    detrended = data - trend[:n]
    seasonal = np.array([np.mean(detrended[i::season_length])
                         for i in range(season_length)])
    # Forecast
    forecast = trend[n:] + np.tile(seasonal, periods_ahead // season_length + 1)[:periods_ahead]
    return forecast

# === Forecast feeds into stockpyl for inventory decisions ===
from stockpyl.newsvendor import newsvendor_normal
# Use forecast mean + std as inputs to inventory optimization
```

For production use, install: `pip install statsmodels` (ARIMA, ETS) or `pip install prophet` (Facebook Prophet).
