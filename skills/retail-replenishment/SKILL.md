...
## Preferred Solver: stockpyl + PuLP

For store-level replenishment parameters, use `stockpyl`. For network-wide replenishment optimization, use `PuLP`:

```python
# === Per-store reorder point and order quantity ===
from stockpyl.rq import r_q_loss_function_approximation

# Calculate for each store given its demand characteristics
r, Q = r_q_loss_function_approximation(
    holding_cost=2,          # $/unit/period at store
    stockout_cost=20,        # lost sale cost
    fixed_cost=50,           # delivery cost per order
    demand_mean=100,         # units/period at this store
    demand_sd=30,            # demand variability
    lead_time=3              # DC-to-store lead time in periods
)

# === Min-Max levels (periodic review) ===
from stockpyl.ss import s_s_power_approximation
s, S = s_s_power_approximation(
    holding_cost=2, stockout_cost=20, fixed_cost=50,
    demand_mean=100, demand_sd=30
)
# Min = s (reorder point), Max = S (order-up-to level)

# === Network-wide: which DC serves which stores ===
import pulp
# Use PuLP to assign store replenishment to DCs minimizing
# transport cost subject to DC capacity constraints
# (see distribution-center-network skill for formulation)
```

Use the skill's frameworks below for choosing replenishment MODEL (min-max, demand-driven, VMI, cross-dock).
