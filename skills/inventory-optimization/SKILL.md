...
For computation, use the `stockpyl` library. It provides validated solvers for common inventory policies:

```python
# === (r, Q) Continuous Review Policy ===
from stockpyl.rq import r_q_loss_function_approximation, r_q_eoqss_approximation

# Loss-function approximation (recommended) — returns (reorder_point, order_quantity)
...
from stockpyl.ss import s_s_power_approximation, s_s_discrete_exact

# Power approximation heuristic — returns (s, S)
s, S = s_s_power_approximation(
    holding_cost=5,
    stockout_cost=50,
    fixed_cost=100,
    demand_mean=1000,
...
# === EOQ (basic order quantity) ===
from stockpyl.eoq import economic_order_quantity
Q, cost = economic_order_quantity(
    fixed_cost=100, holding_cost=12.5, demand_rate=10000
)

# === Newsvendor (single-period / base stock) ===
from stockpyl.newsvendor import newsvendor_normal
base_stock, cost = newsvendor_normal(
    holding_cost=15, stockout_cost=10, demand_mean=200, demand_sd=50
)

# === Finite Horizon DP ===
from stockpyl.finite_horizon import finite_horizon_dp
# For multi-period stochastic inventory with finite horizon

# === Simulation ===
from stockpyl.sim import simulation
# Simulate any supply chain network with configured policies
```

Use the skill's custom code below only for scenarios stockpyl doesn't cover (e.g., ABC/XYZ classification, demand profiling, inventory turns analysis, multi-item budget constraints).
