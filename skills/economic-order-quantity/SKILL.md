...
## Preferred Solver: stockpyl

For computation, use the `stockpyl` library. It provides validated, tested implementations:

```python
from stockpyl.eoq import (
    economic_order_quantity,
    economic_order_quantity_with_backorders,
    ...
    joint_replenishment_problem_silver_heuristic
)

# Basic EOQ
Q, cost = economic_order_quantity(fixed_cost=100, holding_cost=12.5, demand_rate=10000)

# EOQ with backorders (returns: order_qty, stockout_fraction, cost)
Q, stockout_frac, cost = economic_order_quantity_with_backorders(
    fixed_cost=100, holding_cost=12.5, stockout_cost=5.0, demand_rate=10000
)

# EOQ with all-units quantity discounts (returns: order_qty, discount_tier_index, cost)
Q, tier, cost = economic_order_quantity_with_all_units_discounts(
    fixed_cost=100, holding_cost_rate=0.25, demand_rate=10000,
    breakpoints=[0, 500, 1000], unit_costs=[50, 48, 45]
)

# Economic Production Quantity (EPQ)
Q, cost = economic_production_quantity(
    fixed_cost=500, holding_cost=12.5, demand_rate=10000, production_rate=50000
)
```

Use the skill's custom code below only for scenarios stockpyl doesn't cover (e.g., multi-item constrained EOQ, sensitivity analysis plots, reorder point calculations).
