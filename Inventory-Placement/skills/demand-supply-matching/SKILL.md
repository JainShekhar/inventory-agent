...
## Preferred Solver: PuLP (LP/MIP) + stockpyl

For allocation optimization, use `PuLP`. For inventory-aware allocation, combine with `stockpyl`:

```python
import pulp

def solve_supply_allocation(
    supply,             # supply[j]: available inventory at source j
    demand,             # demand[i]: demand at destination i
    costs,              # costs[i][j]: cost to serve i from j
    priorities=None     # priorities[i]: customer priority weight (higher = more important)
):
    """Allocate limited supply to demand locations, minimizing cost."""
    n_demand = len(demand)
    n_supply = len(supply)
    if priorities is None:
        priorities = [1.0] * n_demand

    prob = pulp.LpProblem("SupplyAllocation", pulp.LpMinimize)

    x = [[pulp.LpVariable(f"x_{i}_{j}", lowBound=0) for j in range(n_supply)]
         for i in range(n_demand)]

    # Minimize transport cost + penalty for unmet demand
    unmet = [pulp.LpVariable(f"unmet_{i}", lowBound=0) for i in range(n_demand)]

    prob += (pulp.lpSum(costs[i][j] * x[i][j]
                        for i in range(n_demand) for j in range(n_supply)) +
             pulp.lpSum(priorities[i] * 1000 * unmet[i] for i in range(n_demand)))

    for j in range(n_supply):
        prob += pulp.lpSum(x[i][j] for i in range(n_demand)) <= supply[j]
    for i in range(n_demand):
        prob += pulp.lpSum(x[i][j] for j in range(n_supply)) + unmet[i] == demand[i]

    prob.solve(pulp.PULP_CBC_CMD(msg=0))
    allocations = [[x[i][j].value() for j in range(n_supply)] for i in range(n_demand)]
    return allocations, pulp.value(prob.objective)
```

For determining HOW MUCH to stock at each source (feeds supply[] above):
```python
from stockpyl.newsvendor import newsvendor_normal       # single-period stocking
from stockpyl.rq import r_q_loss_function_approximation # continuous review
from stockpyl.ss import s_s_power_approximation         # periodic review
```
