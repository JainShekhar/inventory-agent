...
For computation, use the `stockpyl` library. It provides a validated Wagner-Whitin solver via dynamic programming:

```python
from stockpyl.wagner_whitin import wagner_whitin

# Multi-period lot sizing with deterministic demand
Q, cost, theta, s = wagner_whitin(
    num_periods=6,
    holding_cost=2,       # $/unit/period (scalar or list)
    fixed_cost=500,       # $/order (scalar or list)
    demand=[100, 120, 80, 90, 110, 70],
    purchase_cost=0       # $/unit (scalar or list)
)
# Q = order quantities per period (Q[t]=0 means no order in period t)
# cost = total optimal cost
# theta = cost-to-go array (DP table)
# s = next-order-period pointers

# For EOQ-based approaches (constant demand):
from stockpyl.eoq import economic_order_quantity
Q_eoq, cost_eoq = economic_order_quantity(
    fixed_cost=500, holding_cost=2, demand_rate=1000
)
```

Use the skill's custom code below only for scenarios stockpyl doesn't cover (e.g., capacity-constrained lot sizing, Silver-Meal heuristic, part-period balancing, multi-item lot sizing).
