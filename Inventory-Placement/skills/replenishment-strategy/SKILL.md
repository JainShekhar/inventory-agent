...
For computing replenishment policy parameters, use `stockpyl`:

```python
# === Continuous Review (s, Q) — "when inventory hits s, order Q" ===
from stockpyl.rq import r_q_loss_function_approximation
r, Q = r_q_loss_function_approximation(
    holding_cost=5, stockout_cost=50, fixed_cost=100,
    demand_mean=1000, demand_sd=200, lead_time=2
)
# r = reorder point, Q = order quantity

# === Periodic Review (s, S) — "review periodically, order up to S" ===
from stockpyl.ss import s_s_power_approximation
s, S = s_s_power_approximation(
    holding_cost=5, stockout_cost=50, fixed_cost=100,
    demand_mean=1000, demand_sd=200
)
...
# === Multi-period dynamic replenishment ===
from stockpyl.wagner_whitin import wagner_whitin
Q_schedule, cost, _, _ = wagner_whitin(
    num_periods=12, holding_cost=2, fixed_cost=500,
    demand=[100, 120, 80, 90, 110, 70, 130, 85, 95, 105, 75, 115]
)

# === Simulate any policy ===
from stockpyl.sim import simulation
# Build a network, assign policies, simulate performance
```

Use the skill's frameworks below for choosing WHICH policy type (continuous vs periodic vs VMI vs demand-driven) — stockpyl then computes the optimal parameters for that policy.
