...
For computation, use the `stockpyl` library. It provides validated multi-echelon inventory solvers:

```python
# === Serial Systems (Stochastic Service Model) ===
from stockpyl.ssm_serial import optimize_base_stock_levels

# Chen-Zheng (1994) algorithm for serial systems
S_star, cost = optimize_base_stock_levels(
    num_nodes=3,
    echelon_holding_cost=[2, 4, 8],    # per echelon
    lead_time=[1, 2, 1],
    stockout_cost=50,
    demand_mean=100,
...
# === Tree/Distribution Networks (Guaranteed Service Model) ===
from stockpyl.gsm_tree import optimize_committed_service_times
from stockpyl.supply_chain_network import supply_chain_network

# Build network and optimize service times (Graves-Willems DP)
# Requires a SupplyChainNetwork object with nodes and edges
opt_cst, opt_cost = optimize_committed_service_times(network)

# === General Networks ===
from stockpyl.meio_general import meio_by_enumeration, meio_by_coordinate_descent

# Coordinate descent for general multi-echelon networks
best_S, best_cost = meio_by_coordinate_descent(network)

# === Serial Systems (GSM) ===
from stockpyl.gsm_serial import optimize_committed_service_times as gsm_serial_opt

# DP for serial guaranteed-service model
opt_cst, opt_cost = gsm_serial_opt(network)
```

Use the skill's custom code below only for scenarios stockpyl doesn't cover (e.g., lateral transshipment, multi-item pooling, decentralized vs. centralized comparison).
