...
Hub location is a MIP. Use `PuLP` with CBC:

```python
import pulp

def solve_p_hub_median(
    n_nodes,            # number of origin/destination nodes
    p,                  # number of hubs to open
    flows,              # flows[i][j]: demand flow from i to j
    distances,          # distances[i][j]: distance between nodes
    hub_discount=0.75   # inter-hub transport discount factor (economies of scale)
):
    """P-hub median: locate p hubs to minimize total transport cost."""
    prob = pulp.LpProblem("PHubMedian", pulp.LpMinimize)
    nodes = range(n_nodes)

    # Binary: is node k a hub?
    h = {k: pulp.LpVariable(f"hub_{k}", cat="Binary") for k in nodes}
    # Flow assignment: node i assigned to hub k
    z = {(i, k): pulp.LpVariable(f"assign_{i}_{k}", cat="Binary")
         for i in nodes for k in nodes}

    # Objective: minimize total transport cost (collection + transfer + distribution)
    prob += pulp.lpSum(
        flows[i][j] * distances[i][k] * z[i, k]  # collection
        for i in nodes for j in nodes for k in nodes
    ) + pulp.lpSum(
        flows[i][j] * hub_discount * distances[k][l] * z[i, k] * z[j, l]
        for i in nodes for j in nodes for k in nodes for l in nodes
    )  # Note: linearize for large instances

    # Exactly p hubs
    prob += pulp.lpSum(h[k] for k in nodes) == p

    # Each node assigned to exactly one hub
    for i in nodes:
        prob += pulp.lpSum(z[i, k] for k in nodes) == 1
    ...
    hubs = [k for k in nodes if h[k].value() > 0.5]
    assignments = {i: [k for k in nodes if z[i, k].value() > 0.5][0] for i in nodes}
    return hubs, assignments, pulp.value(prob.objective)
```

Note: For large instances (>20 nodes), use heuristics or decomposition methods from the skill's code below.
