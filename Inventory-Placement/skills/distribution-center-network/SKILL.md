...
## Preferred Solver: PuLP (MIP) + stockpyl

For network flow optimization, use `PuLP`. For inventory positioning within the network, use `stockpyl`:

```python
import pulp

def solve_dc_network_design(
    plants, dcs, customers,
    dc_fixed_costs,        # cost to open each DC
    dc_capacities,         # throughput capacity per DC
    inbound_costs,         # plant→DC transport cost matrix
    outbound_costs,        # DC→customer transport cost matrix
    plant_capacities,      # supply capacity per plant
    customer_demands       # demand per customer
):
    """Multi-echelon DC network design: Plants → DCs → Customers."""
    prob = pulp.LpProblem("DC_Network", pulp.LpMinimize)

    # Binary: open DC j?
    y = {j: pulp.LpVariable(f"open_{j}", cat="Binary") for j in dcs}
    # Continuous: flow from plant p to DC j
    x_in = {(p, j): pulp.LpVariable(f"in_{p}_{j}", lowBound=0)
             for p in plants for j in dcs}
    # Continuous: flow from DC j to customer c
    x_out = {(j, c): pulp.LpVariable(f"out_{j}_{c}", lowBound=0)
              for j in dcs for c in customers}

    # Objective
    prob += (pulp.lpSum(dc_fixed_costs[j] * y[j] for j in dcs) +
             pulp.lpSum(inbound_costs[p][j] * x_in[p, j] for p in plants for j in dcs) +
             pulp.lpSum(outbound_costs[j][c] * x_out[j, c] for j in dcs for c in customers))

    # Demand satisfaction
    for c in customers:
        prob += pulp.lpSum(x_out[j, c] for j in dcs) == customer_demands[c]

    # DC flow balance
    for j in dcs:
        prob += (pulp.lpSum(x_in[p, j] for p in plants) ==
                 pulp.lpSum(x_out[j, c] for c in customers))

    # DC capacity
    for j in dcs:
        prob += pulp.lpSum(x_out[j, c] for c in customers) <= dc_capacities[j] * y[j]

    # Plant capacity
    for p in plants:
        prob += pulp.lpSum(x_in[p, j] for j in dcs) <= plant_capacities[p]

    prob.solve(pulp.PULP_CBC_CMD(msg=0))
    return {j: y[j].value() for j in dcs}, pulp.value(prob.objective)
```

For inventory positioning at each DC, use stockpyl:
```python
from stockpyl.ssm_serial import optimize_base_stock_levels   # serial systems
from stockpyl.gsm_tree import optimize_committed_service_times  # tree networks
from stockpyl.meio_general import meio_by_coordinate_descent    # general networks
```
