...
The IRP combines routing (PuLP MIP) with inventory decisions (stockpyl):

```python
import pulp

def solve_single_period_irp(
    depot,                 # depot location index
    customers,             # list of customer indices
    demands,               # demand[i]: demand at customer i this period
    inventories,           # inv[i]: current inventory at customer i
    capacities,            # cap[i]: storage capacity at customer i
    vehicle_capacity,      # max load per vehicle
    distances              # dist[i][j]: distance matrix
):
    """Single-period IRP: decide which customers to visit and how much to deliver."""
    prob = pulp.LpProblem("IRP", pulp.LpMinimize)

    # Binary: visit customer i?
    y = {i: pulp.LpVariable(f"visit_{i}", cat="Binary") for i in customers}
    # Continuous: quantity delivered to customer i
    q = {i: pulp.LpVariable(f"qty_{i}", lowBound=0) for i in customers}

    # Objective: minimize routing cost (simplified) + inventory holding
    prob += (pulp.lpSum(distances[depot][i] * 2 * y[i] for i in customers) +
             pulp.lpSum(0.5 * (inventories[i] + q[i] - demands[i])
                        for i in customers))

    # Deliver only if visited
    for i in customers:
        prob += q[i] <= capacities[i] * y[i]

    # Don't exceed storage capacity
    for i in customers:
        prob += inventories[i] + q[i] <= capacities[i]

    # Vehicle capacity
    prob += pulp.lpSum(q[i] for i in customers) <= vehicle_capacity

    # Prevent stockout
    for i in customers:
        prob += inventories[i] + q[i] - demands[i] >= 0

    prob.solve(pulp.PULP_CBC_CMD(msg=0))
    visits = [i for i in customers if y[i].value() > 0.5]
    deliveries = {i: q[i].value() for i in visits}
    return visits, deliveries, pulp.value(prob.objective)
```

For the inventory component (when to replenish, safety stock levels):
```python
from stockpyl.rq import r_q_loss_function_approximation  # reorder triggers
from stockpyl.eoq import economic_order_quantity          # delivery quantities
from stockpyl.wagner_whitin import wagner_whitin          # multi-period schedule
```
